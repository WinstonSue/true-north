...
import { TodoRelatedType, TodoStatus, TrackTimeRelatedType } from '@true-north/enum';
...
import { DEFAULT_PLAN_TIME, defaultTimeRange, isPlanRange, normalizePlanRange, toDayjsTime } from './planTime';
import { WorkflowAssociationField } from '../../../integrations/workflow-association';
import { TodoNotifyFields } from '../../../shared/notify/NotifyRuleFields';
import { useFocusTimer } from '../../../runtime/focus-timer';
...
function toFormPatch(changedValues: Partial<TodoFormValues>): Partial<TodoFormData> {
  ...
  if ('description' in changedValues) {
    patch.description = typeof changedValues.description === 'string' ? changedValues.description : undefined;
  }
...
function toTodoRepeat(value: RepeatSelectorValue | undefined, planDate: string): TodoFormData['repeatConfig'] {
  return value
    ? ({
        ...value,
...
export default function TodoForm(props: TodoFormProps) {
  const { mode, todoFormData, setTodoFormData, onSubmit, currentTodo } = useTodoDetailContext();
  const { open: openFocusTimer } = useFocusTimer();
  const [form] = Form.useForm<TodoFormValues>();
  const [moreOpen, setMoreOpen] = useState(() => Boolean(todoFormData.description));
  ...
  useEffect(() => {
    ...
    void TrackTimeController.findByRelatedId(TrackTimeRelatedType.TODO, currentTodo.id)
      .then((response) => setTrackRecords(response?.list || []))
      .catch(() => setTrackRecords([]));
  }, [mode, currentTodo?.id]);

  const isEditor = mode === 'editor';
  const isCreate = !isEditor;
  const hideAdvanced = isCreate && !moreOpen;
  const repeatValue = todoFormData.repeatConfig as RepeatSelectorValue | undefined;
  const [planStart, planEnd] = normalizePlanRange(todoFormData.planTimeRange ?? [DEFAULT_PLAN_TIME, DEFAULT_PLAN_TIME]);
  ...
  const canFocus = isEditor && currentTodo?.status === TodoStatus.TODO && isPlanRange(planStart, planEnd);
  const isPointPlan = !isPlanRange(planStart, planEnd);
  ...
  return (
    <Flex vertical className={styles.container}>
            ...
            patch.repeatConfig = toTodoRepeat(
              {
                ...
                repeatStartDate: settledTimes ? todoFormData.repeatConfig.repeatStartDate : patch.planDate,
      ...
      >
        <Form.Item label="待办名称" name="name" rules={[{ required: true, message: '请输入待办名称' }]}>
        ...
        >
          {!repeatValue && <DatePicker className="w-full" format="YYYY-MM-DD" allowClear={false} />}
          ...
          label={
            <Flex justify="space-between" align="center" gap={8}>
              ...
              <Radio.Group
                size="small"
                optionType="button"
                value={isPlanRange(planStart, planEnd) ? 'range' : 'point'}
                onChange={(event) => {
        ...
        <Form.Item label="完成时发起流程">
          <WorkflowAssociationField
            value={todoFormData.workflowDefinitionId}
            onChange={(workflowDefinitionId) => setTodoFormData({ workflowDefinitionId })}
          />
        </Form.Item>

        <TodoNotifyFields
          inherit={!todoFormData.notifyRule}
          value={todoFormData.notifyRule}
          onChange={(notifyRule) => setTodoFormData({ notifyRule })}
        />
              ...
              {trackRecords.length ? (
                trackRecords.map((record) => (
                  <Flex key={record.id} justify="space-between" align="center">
                    <span>{record.startAt ? dayjs(record.startAt).format('YYYY-MM-DD HH:mm') : '专注记录'}</span>
      ...
      <Flex className={styles.footer} justify="flex-end" gap={8}>
        {props.onClose ? <Button onClick={() => props.onClose?.()}>取消</Button> : null}
