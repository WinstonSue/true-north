...
import { DEFAULT_PLAN_TIME, normalizePlanRange } from './planTime';
import { useRendererPlatform } from '@true-north/plugin-sdk/renderer';
import { loadTodoAssociation, saveTodoAssociation } from '../../../integrations/workflow-association';

export type TodoSubmitCreate = (
  form: TodoFormData,
...
export function todoFormDataToCreateVo(form: TodoFormData): CreateTodoVo | null {
  const name = toTextValue(form.name).trim();
  if (!name) return null;
  const [planStartTime, planEndTime] = normalizePlanRange(form.planTimeRange);
  ...
  return {
    ...
    repeatConfig,
    notifyRule: form.notifyRule ?? null,
...
export const [TodoDetailProvider, useTodoDetailContext] = createInjectState<{
  ...
  const defaultFormData: TodoFormData = {
    ...
    planTimeRange: normalizePlanRange(
  ...
  const refreshTodoDetail = async (
    ...
    todoFormDataRef.current = {
      ...formData,
      planTimeRange: normalizePlanRange(formData.planTimeRange),
      workflowDefinitionId: await loadTodoAssociation(platform.ipc, id),
    };
  ...
  async function handleCreate(): Promise<boolean> {
    ...
    if (!vo) return false;
    try {
      if (props.submitCreate) {
        const result = await props.submitCreate(form);
        const ok = result === true || (typeof result === 'object' && result !== null);
        if (!ok) return false;
        emitTodoChanged();
        return true;
      }
      const created = await TodoService.create(vo);
      if (created?.id) {
        await saveTodoAssociation(platform.ipc, created.id, form.workflowDefinitionId || null);
      }
  ...
  async function handleUpdate(): Promise<boolean> {
    ...
    const [planStartTime, planEndTime] = normalizePlanRange(
      todoFormDataRef.current.planTimeRange,
    );
    const data = TodoMapping.formDataToUpdateVo({
      ...todoFormDataRef.current,
      name,
      description: optionalText(todoFormDataRef.current.description),
      planTimeRange: [planStartTime, planEndTime],
    });
    try {
      await TodoService.update(currentTodo.relatedType, currentTodo.id, data);
      await saveTodoAssociation(platform.ipc, currentTodo.id, todoFormDataRef.current.workflowDefinitionId || null);
      emitTodoChanged();
      return true;
    } catch (error) {
      console.error(error);
  ...
  return {
    ...
    setTodoFormData: (formData: Partial<TodoFormData>) => {
      ...
      if ('description' in normalizedFormData) {
        normalizedFormData.description = optionalText(normalizedFormData.description);
      }
      if ('planTimeRange' in normalizedFormData && normalizedFormData.planTimeRange) {
        normalizedFormData.planTimeRange = normalizePlanRange(
