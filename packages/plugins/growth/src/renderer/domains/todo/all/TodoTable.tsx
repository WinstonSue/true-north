...
import { URGENCY_MAP, IMPORTANCE_MAP } from '../../../shared/constants';
...
import {
  confirmTodoWorkflowDecision,
  saveTodoAssociation,
  useOpenPendingWorkflows,
      ...
      render: (_, record) => {
        ...
        return (
          <div>
            ...
            {record.status === TodoStatus.DONE && (
              <Button
                type="text"
                onClick={async () => {
                  const decision = await confirmTodoWorkflowDecision({
                    ipc: platform.ipc,
                    ownerId: record.id,
                    title: '撤回完成',
                    content:
                      '该待办完成时可能已发起流程。回滚会撤销已产生的账单等结果；保留则会把流程实例从待办上拆开，工作台和账单仍在。',
                  });
                  if (decision === 'cancel') return;
                  await TodoService.restore(relatedType, record.id);
                  emitTodoChanged();
                  await getTodoPage();
                }}
              >
                ...
                onClick={() => {
                  ...
                  Modal.confirm({
                    ...
                    content: isRepeatSeries
                      ? '将停止后续重复生成，已完成/已放弃的历史记录会保留。已关联的流程只停止以后触发，不回滚历史实例。'
                      : '删除后无法恢复。若该待办已完成并发起过流程，请在下一步选择回滚、保留结果或取消。',
                    okText: '删除',
                    cancelText: '取消',
                    onOk: async () => {
                      try {
                        if (isRepeatSeries) {
                          await saveTodoAssociation(platform.ipc, record.id, null);
                        } else {
                          const decision = await confirmTodoWorkflowDecision({
                            ipc: platform.ipc,
                            ownerId: record.id,
                            title: '删除待办',
                            content:
                              '该待办可能已发起流程。回滚会撤销已产生的账单等结果；保留则会把流程实例从待办上拆开。',
                          });
                          if (decision === 'cancel') return;
                        }
