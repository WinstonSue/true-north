...
import { HabitNotifyFields } from '../../../shared/notify/NotifyRuleFields';
...
export const CreateHabit: React.FC<CreateHabitProps> = ({
  ...
  const [repeatSetting, setRepeatSetting] = useState<RepeatSelectorValue>(() =>
    createDefaultRepeatSetting(
      initialValues?.repeatStartDate || dayjs().format('YYYY-MM-DD'),
    )
  );
  const [notifyRule, setNotifyRule] = useState<HabitNotifyRule | null>(habit?.notifyRule ?? null);
  ...
  useEffect(() => {
    ...
    setNotifyRule(habit.notifyRule ?? null);
  }, [form, habit]);
  ...
  return (
    <div>
      ...
      >
        ...
        <HabitNotifyFields inherit={!notifyRule} value={notifyRule} onChange={setNotifyRule} />
