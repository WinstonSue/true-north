...
import { emitTaskChanged } from '../../../shared/events';
...
export function normalizePlanRange(
...
function normalizeFormData(formData: TaskFormData): TaskFormData {
  return {
    ...formData,
...
export const [TaskDetailProvider, useTaskDetailContext] = createInjectState<{
  ...
  const defaultFormData = normalizeFormData({
    name: '',
    children: [],
    trackTimeList: [],
  ...
  const refreshTaskDetail = async (id: string) => {
    ...
    setCurrentTask(task);
    setTaskFormData(normalizeFormData(TaskMapping.voToFormData(task)));
  };
