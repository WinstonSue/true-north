import { Input, InputNumber, DatePicker, Switch, Spin, Select, Form, Button, type FormRule, Row, Col } from '@sue/design-web-react';
...
import GoalTreeSelector from '../../goal/GoalTreeSelector';
import { useTaskFormConstraints } from './hooks';
import { TaskService, GoalService } from '../../../../client';
import { DIFFICULTY_MAP, IMPORTANCE_MAP } from '../../../shared/constants';
import { TaskNotifyFields } from '../../../shared/notify/NotifyRuleFields';
...
export default function TaskForm() {
  ...
  const hideAdvanced = isCreateMode && !moreOpen;
  ...
  const planRange = useMemo(
    () => normalizePlanRange(taskFormData.planTimeRange),
  ...
  useEffect(() => {
    if (!loading && currentTask?.id) {
      form.setFieldsValue({
        ...taskFormData,
        planTimeRange: planRange,
      });
    }
  }, [currentTask?.id, form, loading, planRange, taskFormData]);
  ...
  return (
    <Form
      form={form}
      initialValues={{ ...taskFormData, planTimeRange: planRange }}
    ...
    >
      <Row gutter={[16, 16]} className="p-2">
        ...
        {isCreateMode ? (
          <Col span={24} className="w-full !p-0">
        ...
        <Col span={24} className="w-full !p-0">
          <TaskNotifyFields
            inherit={!taskFormData.notifyRule}
            value={taskFormData.notifyRule}
            onChange={(notifyRule) => setTaskFormData((prev) => ({ ...prev, notifyRule }))}
          />
        </Col>
        <div className={hideAdvanced ? 'hidden w-full' : 'contents'}>
