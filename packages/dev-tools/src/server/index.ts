...
import { batchCheckControllerSync } from '../watch-controllers/sync/enhanced-sync'
...
function findControllerPairV2(name: string) {
  ...
  return pairs.find(p => p.name.toLowerCase() === name.toLowerCase() || 
                         p.className.toLowerCase().includes(name.toLowerCase())) || null
}
...
app.get('/api/check/method-details', async (req, res) => {
  try {
    ...
    // 执行方法级别的差异检测
    const detailedStatuses = batchCheckControllerSync(controllers)
    
    res.json({ 
      success: true, 
      data: {
        controllers: detailedStatuses,
  ...
  try {
    ...
    res.json({ 
      ...
      message: result.success ? 
        (dryRun ? `${pair.className} 检查完成` : `${pair.className} 同步完成`) : 
        `${pair.className} 同步失败`,
      error: result.error
    })
  } catch (error) {
