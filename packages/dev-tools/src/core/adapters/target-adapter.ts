import { IntermediateState, ControllerMetadata, MethodDefinition, ConstructorDefinition, ImportDeclaration, ParameterDefinition, SourceLocation } from '../intermediate-state';
import { ASTParser } from '../ast/ast-parser';
import { ASTClassInfo, ASTMethod, ASTDecorator, ASTParameter, ASTConstructor, ASTImport, ASTSourceLocation } from '../ast/ast-types';
import { TargetAdapter as ITargetAdapter } from './adapter-types';

export class TargetAdapter implements ITargetAdapter {
   ...
   * 生成方法体哈希
   */
  private generateBodyHash(bodyText: string): string {
    // 简单的哈希实现
    let hash = 0;
