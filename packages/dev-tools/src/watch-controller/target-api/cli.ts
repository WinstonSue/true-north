...
/**
 * API 控制器同步 CLI 工具
 * 使用新架构的同步引擎进行 API 控制器同步
 */

import { Command } from 'commander';
import { join } from 'path';
import { existsSync } from 'fs';
import { createApiSyncEngine } from './sync-engine.js';
import { CONTROLLER_SOURCE_PATH, CONTROLLER_API_TARGET_PATH } from '../../constants.js';
...
function findControllerPairs(): ControllerPair[] {
  ...
  for (const controller of controllers) {
    ...
    const targetPath = join(CONTROLLER_API_TARGET_PATH, controller.path, `${controller.name}.controller.ts`);
...
async function syncCommand(controllerName?: string, options: any = {}) {
  ...
  try {
    ...
    if (controllerName) {
      ...
      console.log(`🎯 同步 API 控制器: ${pair.className}`);
      console.log(`   Server: ${pair.sourcePath}`);
      console.log(`   API: ${pair.targetPath}\n`);
      ...
      // 同步所有控制器
      const pairs = findControllerPairs();
      console.log(`📋 发现 ${pairs.length} 个 API 控制器对\n`);
...
program
  .name('api-sync')
  .description('API 控制器同步 CLI')
  .version('1.0.0');

program
  .command('sync')
  .description('同步 API 控制器')
