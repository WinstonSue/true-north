/**
 * 基于正则表达式的 Controller 解析器
 * 提供传统的字符串匹配和解析功能
 */
import { MethodDecoratorInfo, ClassBodyRange, MethodOccurrence } from '../types';
import { findMethodBodyOpenBraceIndex, findAllTopLevelMethodBlocks } from './text-scanner';
...
export function getMethodOccurrences(body: string, name: string): Array<{ start: number; end: number }> {
...
export function getClassBodyRange(content: string, className: string): { start: number; end: number } | null {
...
export function parseApiMethodNames(apiContent: string, className: string): Set<string> {
...
export function parseMethodDecorators(sourceContent: string, className: string): Map<string, MethodDecoratorInfo> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 匹配装饰器行
    const decoratorMatch = line.match(/^@(Get|Post|Put|Delete)\s*\(\s*(.+)\s*\)\s*$/);
    if (decoratorMatch) {
      currentDecorator = {
    ...
    if (methodMatch && currentDecorator) {
      ...
      if (fullMethodLine.includes('(') && !fullMethodLine.match(/\(\s*\)/)) {
        ...
        const hasBodyParam =
          ...
          (fullMethodLine.match(/\([^)]*,/g) && currentDecorator.verb.toLowerCase() !== 'get');
          ...
          for (const param of params) {
            if (param.includes('id:')) {
              ...
              if (typeMatch) {
                ...
                if (hasQueryParam || currentDecorator.verb.toLowerCase() === 'get') {
        ...
        if (hasIdParam && hasBodyParam) {
          ...
          paramStyle = 'id';
        } else if (hasQueryParam || currentDecorator.verb.toLowerCase() === 'get') {
          paramStyle = 'query';
        } else {
          paramStyle = 'body';
        }
...
export function getApiClassBodyRange(content: string, className: string): { start: number; end: number } | null {
  const classRegex = new RegExp(`export\\s+default\\s+class\\s+${className}\\s*\\{`);
