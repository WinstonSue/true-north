/**
 * 基于 ts-morph 的 AST 解析器核心实现
 * 提供更准确的 TypeScript 代码解析能力
 */
import {
  Project,
  SourceFile,
...
import { ASTMethodDecoratorInfo, ASTControllerInfo, ASTClassBodyRange } from './types';
...
export class TypeScriptASTParser {
  ...
  parseControllerInfo(content: string, className?: string): ASTControllerInfo | null {
    ...
    // 解析构造函数服务类型
    const constructorServiceTypes = this.parseConstructorServiceTypes(content);

    return {
      className: classDeclaration.getName() || 'UnknownController',
      basePath,
      methods: methodMap,
      constructorServiceTypes,
    };
  }
