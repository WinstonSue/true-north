...
export class ProxyCodeGenerator {
  /**
   * 应用同步操作到目标代码
   */
  applySyncActions(
  ...
  private updateMethod(code: string, method: MethodDefinition, targetState: IntermediateState): string {
    // 先移除旧方法
    const methodRange = this.findMethodRange(code, method.name);
    if (methodRange) {
    ...
    const methodCode = this.generateDesktopMethod(method);
    const insertion = '\n' + methodCode + '\n';

    const beforeClosing = code.slice(0, classBodyRange.end);
    const afterClosing = code.slice(classBodyRange.end);

    return beforeClosing + insertion + afterClosing;
  }

  /**
   * 移除方法
   */
  private removeMethod(code: string, methodName: string, targetState: IntermediateState): string {
