...
| Feature | @sue/design-web-vue version | Compatibility | Minimum Chrome Version | Compatibility workaround |
| --- |-------------------| --- | --- | --- |
...
If you need to support older browsers, please use `@sue/cssinjs` [StyleProvider] for degradation handling according to your actual requirements.
...
To unify LTR and RTL styles, Antdv Next uses CSS logical properties. For example, the original `margin-left` is replaced by `margin-inline-start`, so that it is the starting position spacing under both LTR and RTL. If you need to be compatible with older browsers (such as 360 Browser, QQ Browser, etc.), you can configure `transformers` through the `StyleProvider` of `@sue/cssinjs`:

```vue
<script lang="ts" setup>
import { legacyLogicalPropertiesTransformer } from '@sue/cssinjs'
</script>

<template>
  <!-- `transformers` provides preprocessing to transform styles -->
  <sue-style-provider :transformers="[legacyLogicalPropertiesTransformer]">
    <MyApp />
  </sue-style-provider>
</template>
```
...
When `zeroRuntime` is enabled, Antdv Next’s styles are precompiled into a standalone `sue.css` file. If you also enable the `@layer` specificity–lowering mechanism, you must ensure that `sue.css` is placed inside the same layer (e.g., `layer(sue)`). Otherwise, its specificity will be higher than the styles injected by StyleProvider, causing the lowering mechanism to fail or resulting in unexpected override behavior.

```css
/* global.css / app.css */
@layer theme, base, antd, components, utilities;

/* The precompiled sue.css output by zeroRuntime must explicitly specify a layer */
@import url(sue.css) layer(sue);
```
...
```vue
<script lang="ts" setup>
import { autoPrefixTransformer } from '@sue/cssinjs'
</script>

<template>
  <sue-style-provider :transformers="[autoPrefixTransformer]">
    <MyApp />
  </sue-style-provider>
</template>
```
...
```vue
<script lang="ts" setup>
import { px2remTransformer } from '@sue/cssinjs'
...
<template>
  <sue-style-provider :transformers="[px2rem]">
    <MyApp />
  </sue-style-provider>
</template>
```
...
Since `<style />` tag insertion is different from normal DOM in Shadow DOM scenario, you need to use `StyleProvider` of `@sue/cssinjs` to configure the `container` property to set the insertion position:
...
### reset.css and sue.css

If you are using Antdv Next’s `reset.css`, you need to assign it to a specific `@layer` to prevent it from overriding the lowered-specificity antd styles. Similarly, in the `zeroRuntime` scenario, if you import `sue.css` separately, you must also place it inside `layer(sue)` to keep the layer hierarchy consistent:

```css
/* Both reset.css and sue.css must specify a layer */
@layer reset, antd;

/* reset styles */
@import url(@sue/design-spec/reset.css) layer(reset);
...
- sue.css (in zeroRuntime mode) stays aligned with the layer injected by StyleProvider
