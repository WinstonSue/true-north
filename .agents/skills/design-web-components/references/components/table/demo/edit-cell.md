...
import type { TableDataIndex, TableProps } from '@sue/design-web-vue'
...
<template>
  <div style="margin-bottom: 16px">
    <sue-button type="primary" @click="handleAdd">
      Add a row
    </sue-button>
  </div>
  <sue-table :columns="columns" :data-source="dataSource" bordered>
    <template #bodyCell="{ column, record, text }">
      <template v-if="column.key === 'operation'">
        <sue-popconfirm title="Sure to delete?" @confirm="handleDelete(record.key)">
          <a>Delete</a>
        </sue-popconfirm>
      </template>
