...
- Use Table to display, compare, sort, filter, paginate, or select structured row data. See `demo/basic.md`, `demo/head.md`, `demo/pagination.md`, and `demo/row-selection.md`.
- Use fixed header/columns, sticky, responsive, narrow, or virtual list demos for large or constrained data. See `demo/fixed-header.md`, `demo/fixed-columns.md`, `demo/sticky.md`, `demo/responsive.md`, `demo/narrow.md`, and `demo/virtual-list.md`.
- Use custom cells, grouped columns, summaries, expandable rows, tree data, or nested tables for richer records. See `demo/cell-slot.md`, `demo/grouping-columns.md`, `demo/summary.md`, `demo/expand.md`, `demo/tree-data.md`, and `demo/nested-table.md`.
- Use editable or draggable demos when users modify table content or order. See `demo/edit-row.md`, `demo/edit-cell.md`, `demo/drag-sorting.md`, and `demo/resizable-column.md`.
- Prefer List/Card layouts when records are not naturally tabular.
...
| full | Fill the height of parent container. Parent must provide an explicit height. For non-virtual tables, body becomes the vertical scroll area when `scroll.y` is not set; explicit `scroll.y` keeps priority | boolean | false | - |
| getPopupContainer | The render container of dropdowns in table| (triggerNode) => HTMLElement |  => TableHtmlElement | - |
| loading | Loading status of table | boolean \| [Spin Props](../spin/docs.md#props) | false | - |
| locale | The i18n text including filter, sort, empty text, etc | object | [默认值] | - |
...
| filterDropdown | Customized filter overlay | VueNode \| (props: [FilterDropdownProps]) => VueNode | - | - |
...
| y | Set vertical scrolling and the scroll area height. When omitted on a non-virtual `full` table, an adaptive height is used internally | string \| number | - |
