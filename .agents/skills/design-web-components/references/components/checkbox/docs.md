...
- Use Checkbox for binary choices or multi-select option groups. See `demo/basic.md` and `demo/group.md`.
- Use check-all behavior when users need bulk selection. See `demo/check-all.md`.
- Use controlled Checkbox when selection is driven by external state. See `demo/controller.md`.
- Prefer Switch for immediate on/off settings and Radio for mutually exclusive choices.
