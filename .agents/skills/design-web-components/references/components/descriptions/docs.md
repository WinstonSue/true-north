...
- Use Descriptions to present read-only key-value details for an entity, record, or summary. See `demo/basic.md`.
- Use bordered, vertical, or responsive layouts when detail density or viewport size changes. See `demo/border.md`, `demo/vertical.md`, and `demo/responsive.md`.
- Use block or size demos to tune dense admin pages. See `demo/block.md` and `demo/size.md`.
- Prefer Form for editable data and Table for many records.
...
| size | Set the size of the list. Can be set to `medium` or `small` | 'medium' \| 'small' | - | - |
