...
- Use AutoComplete when users can type free text while receiving selectable suggestions. See `demo/basic.md`.
- Use `options` for simple suggestion lists and custom rendering for grouped or rich suggestions. See `demo/options.md`, `demo/certain-category.md`, and `demo/custom.md`.
- Use case-insensitive or custom filtering when search behavior should differ from exact matching. See `demo/non-case-sensitive.md`.
- Prefer Select when users must choose only from fixed options; prefer Input when suggestions are unnecessary.
