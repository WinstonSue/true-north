...
import type { TableProps, TransferEmits, TransferProps } from '@sue/design-web-vue'
import { Table } from '@sue/design-web-vue'
import { ref } from 'vue'
  ...
  return (record) => {
    ...
    return {
      onClick: (e: MouseEvent) => {
        ...
        if (!el)
          return
        if (el.closest('.sue-table-selection-column'))
          return
        const key = row.key
...
<template>
  <div>
    ...
    <sue-space align="center" style="margin-top: 12px">
      <sue-switch
