...
- Use Transfer for moving multiple items between source and target lists. See `demo/basic.md`.
- Use one-way, search, pagination, custom actions, or custom item demos for list-transfer workflows. See `demo/oneWay.md`, `demo/search.md`, `demo/large-data.md`, `demo/actions.md`, and `demo/custom-item.md`.
- Use table or tree transfer when each item needs structured columns or hierarchy. See `demo/table-transfer.md` and `demo/tree-transfer.md`.
- Prefer Select multiple for compact option picking and Table row selection for selecting rows without transfer semantics.
...
If there's no `key` in your data, use `rowKey` to point Transfer at the record's unique identifier.
