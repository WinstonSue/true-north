...
import type { MenuItemType, MenuProps } from '@sue/design-web-vue'
...
<template>
  <sue-config-provider :theme="themeConfig">
    <sue-menu mode="horizontal" :items="menuItems" :popup-render="popupRender" />
  </sue-config-provider>
</template>

<style scoped>
.menu-popup :deep(.sue-menu) {
  display: grid;
...
.menu-popup :deep(.sue-menu-title-content) {
  display: block;
}
