...
- Use ColorPicker when users need to choose or inspect color values. See `demo/basic.md`.
- Use controlled mode, value format, or clear behavior when color values are stored in forms or design tools. See `demo/controlled.md`, `demo/value-format.md`, and `demo/allow-clear.md`.
- Use presets, gradients, or panel customization for design-token and theme editing workflows. See `demo/presets.md`, `demo/line-gradient.md`, and `demo/panel-render.md`.
- Prefer Select or Radio when users choose from a short fixed palette and do not need arbitrary colors.
