...
Wave effect brings dynamic. Use `component` to determine which component uses it.
...
<template>
  <sue-flex gap="large" wrap>
    <sue-config-provider v-for="item in configs" :key="item.name" :wave="item.wave">
