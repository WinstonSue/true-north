...
- Use ConfigProvider at app or subtree boundaries to provide locale, direction, size, theme, popup, virtual scroll, or wave configuration. See `demo/locale.md`, `demo/direction.md`, `demo/size.md`, and `demo/theme.md`.
- Use custom wave configuration when click feedback needs global tuning. See `demo/wave.md`.
- Use holder rendering when static Message, Notification, or Modal APIs need provider context. See `demo/holder-render.md`.
- Use `useConfig` when a custom component needs to read inherited design-web configuration. See `demo/use-config.md`.
...
| iconPrefixCls | Set icon prefix className | string | `sueicon` | - |
...
| prefixCls | Set prefix className | string | `sue` | - |
...
```ts
import { App, ConfigProvider } from '@sue/design-web-vue'
import { h } from 'vue'
...
```ts
import { useConfig } from '@sue/design-web-vue/config-provider/context'
...
| editableText | Set EditableText and EditableParagraph common props | &#123; class?: string, style?: CSSProperties &#125; | - | - |
...
In production mode of Vite, default exports from cjs file should be used like this: `enUS.default`. So you can directly import locale from `es/` directory like `import enUS from '@sue/design-web-vue/es/locale/en_US'` to make dev and production have the same behavior.
