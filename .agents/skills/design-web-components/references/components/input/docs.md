...
- Use Input for short free-text entry. See `demo/basic.md`.
- Use TextArea, Password, Search, or OTP variants for specialized text entry. See `demo/textarea.md`, `demo/password-input.md`, `demo/search-input.md`, and `demo/otp.md`.
- Use allowClear, prefix/suffix, count, status, size, variant, and focus demos for common form behavior. See `demo/allowClear.md`, `demo/presuffix.md`, `demo/show-count.md`, `demo/status.md`, `demo/size.md`, `demo/variant.md`, and `demo/focus.md`.
- Prefer AutoComplete when suggestions are useful and Select when input must be constrained to options.
...
| iconRender | Custom toggle button | (visible) =&gt; VueNode | (visible) =&gt; (visible ? &lt;Eye /> : &lt;EyeOff />) | - |
