...
- Use Tabs to switch between peer content panels within the same context. See `demo/basic.md`.
- Use placement, size, centered, icon, indicator, or extra content demos for presentation variants. See `demo/placement.md`, `demo/size.md`, `demo/centered.md`, `demo/icon.md`, `demo/custom-indicator.md`, and `demo/extra.md`.
- Use card, editable-card, custom add trigger, or custom tab bar demos for workspace-like interfaces. See `demo/card.md`, `demo/editable-card.md`, `demo/custom-add-trigger.md`, and `demo/custom-tab-bar.md`.
- Prefer Segmented for compact mode switches and Anchor for in-page navigation.
...
| addIcon | Customize add icon, only works with `type="editable-card"` | VueNode | `<Plus />` | - |
...
| more | Customize the collapse menu | MoreProps | \{ icon: `<Ellipsis />`, trigger: 'hover' \} | - |
| moreIcon | Custom icon of the collapse menu | VueNode | `<Ellipsis />` | - |
| removeIcon | The custom icon of remove, only works with `type="editable-card"` | VueNode | `<X />` | - |
