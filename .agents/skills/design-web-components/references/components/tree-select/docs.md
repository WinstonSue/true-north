...
- Use TreeSelect when a form field must select one or more values from hierarchical data. See `demo/basic.md` and `demo/multiple.md`.
- Use checkable, treeData, async loading, tree line, or maxCount demos for hierarchical selection behavior. See `demo/checkable.md`, `demo/treeData.md`, `demo/async.md`, `demo/treeLine.md`, and `demo/maxCount.md`.
- Use placement, status, variant, prefix, or suffix demos for form integration. See `demo/placement.md`, `demo/status.md`, `demo/variant.md`, and `demo/suffix.md`.
- Prefer Cascader for step-by-step hierarchical choice and Tree for non-form tree browsing.
...
| suffixIcon | The custom suffix icon | VueNode | `&lt;ChevronDown /&gt;` | - |
