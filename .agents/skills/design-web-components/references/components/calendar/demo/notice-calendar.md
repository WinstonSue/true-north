...
This component can be rendered by using `cellRender` with the data you need.

## Source

```vue
<script setup lang="ts">
import type { BadgeProps } from '@sue/design-web-vue'
      ...
      <template v-if="info.type === 'date'">
        <ul class="events">
          <li v-for="item in getListData(date)" :key="item.content">
            <sue-badge :status="item.type as BadgeProps['status']" :text="item.content" />
          </li>
        </ul>
      </template>
