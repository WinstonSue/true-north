...
import type { BorderBeamGradient } from '@sue/design-web-vue'
...
const presets: Preset[] = [
  ...
  },
  {
    ...
    name: 'Nebula',
    usage: 'Labs',
    description: 'A cool purple-magenta mix that fits experimental modules and product lab surfaces.',
    color: [
...
<template>
  <sue-flex vertical :gap="16" :style="{ maxWidth: '480px' }">
    <sue-segmented
      v-model:value="currentKey"
      block
      :options="segmentOptions"
    />
    <sue-border-beam :color="currentPreset.color">
      <sue-card
        :title="currentPreset.name"
        :styles="{ body: { display: 'flex', flexDirection: 'column', gap: '16px' } }"
      >
        <template #extra>
          <sue-tag variant="filled">
            {{ currentPreset.usage }}
          </sue-tag>
        </template>
