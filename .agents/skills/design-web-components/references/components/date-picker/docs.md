...
- Use DatePicker for choosing or typing a date, month, week, quarter, year, or date range. See `demo/basic.md`, `demo/range-picker.md`, and `demo/switchable.md`.
- Use format or valueFormat when displayed text and bound value need explicit formats. See `demo/format.md` and `demo/value-format.md`.
- Use time, multiple, presets, or needConfirm for richer selection workflows. See `demo/time.md`, `demo/multiple.md`, `demo/preset-ranges.md`, and `demo/needConfirm.md`.
- Use disabled date/time, range limits, or allowEmpty when business rules constrain valid dates. See `demo/disabled-date.md`, `demo/date-range.md`, and `demo/allow-empty.md`.
- Prefer Calendar when users need to browse date content rather than fill a form field.
...
Use ConfigProvider `locale` for application-level localization, or the DatePicker `locale` prop for component-level overrides. See `demo/basic.md` and the Vue i18n docs.
...
| format | To set the date format, support multi-format matching when it is an array, display the first one shall prevail. refer to [dayjs#format](https://day.js.org/docs/en/display/format). for example: [Custom Format](#date-picker-demo-format) | [formatType](#formattype) | [@rc-component/picker] | - |
...
| size | To determine the size of the input box, the height of `large` and `small`, are 40px and 24px respectively, while medium size is 32px | `large` \| `medium` \| `small` | - | - |
...
| presets | The preset ranges for quick selection, Since `5.8.0`, preset value supports callback function. | { label: VueNode, value: (Dayjs \| ( => Dayjs))[] }[] | - | - |
| renderExtraFooter | Render extra footer in panel |  => VueNode | - | - |
| separator | Set separator between inputs | VueNode | `<ArrowRight />` | - |
| showTime | To provide an additional time selection | object \| boolean | [TimePicker Options](../time-picker/docs.md/#api) | - |
...
Please use the correct language package from the Vue i18n docs, or update the Day.js locale configuration before rendering date-related components.
