...
import { FileSpreadsheet, FileText, Loader2, Paperclip, Image, Plus } from '@lucide/vue'
...
const fileSufIconList = [
  { icon: FileText, suf: ['.pdf'] },
  { icon: FileSpreadsheet, suf: ['.xlsx', '.xls', '.csv'] },
  { icon: FileText, suf: ['.doc', '.docx'] },
  {
    icon: Image,
...
const iconRender: UploadProps['iconRender'] = (file, listType) => {
  let icon = file.status === 'uploading' ? h(Loader2) : h(Paperclip)
