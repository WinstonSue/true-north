...
import type { DrawerProps, GetProp } from '@sue/design-web-react';
...
const stylesFn: DrawerProps['styles'] = (): GetProp<DrawerProps, 'styles', 'Return'> => ({
  header: {
    padding: 16,
  },
  body: {
    ...
    <Flex gap="medium">
      ...
      >
        <Flex vertical gap="medium" style={{ height: '100%' }}>
          <div style={{ flex: 1, overflow: 'auto' }}>{sharedContent}</div>
          <Flex gap="medium" justify="flex-end">
            <Button onClick={() => setDrawerFnOpen(false)}>Cancel</Button>
