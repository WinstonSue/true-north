...
If there are special needs (only modifying single component language), Please use the property: locale. Example: [default](https://github.com/yuce-design/yuce-design/blob/master/components/date-picker/locale/example.json).
...
import locale from '@sue/design-web-react/locale/zh_CN';
...
:::warning
When using the Next.js App Router, make sure to add `'use client'` before importing Day.js locale files. This is because all Yuce Design components only work on the client side, so importing locale files in RSC will not work.
:::
...
| locale | Localization configuration | object | [default](https://github.com/yuce-design/yuce-design/blob/master/components/date-picker/locale/example.json) |  | × |
...
| mode | The picker panel mode（ [Cannot select year or month anymore?](../../docs/react/faq.md#when-set-mode-to-datepickerrangepicker-cannot-select-year-or-month-anymore) ) | `time` \| `date` \| `month` \| `year` \| `decade` | - |  | × |
...
| renderExtraFooter | Render extra footer in panel | () => React.ReactNode | - |  | × |
| separator | Set separator between inputs | React.ReactNode | `<ArrowRight />` |  | 6.3.0 |
| showTime | To provide an additional time selection | object \| boolean | [TimePicker Options](/components/time-picker/#api) |  | × |
...
Please refer [FAQ](../../docs/react/faq.md#when-set-mode-to-datepickerrangepicker-cannot-select-year-or-month-anymore)
...
Please refer [Use custom date library](../../docs/react/use-custom-date-library.md#datepicker)
...
Please use correct [language](../../docs/react/i18n.md) ([#5605](https://github.com/yuce-design/yuce-design/issues/5605)), or update dayjs `locale` config:
...
When you change the layout of nodes by `panelRender`, React will unmount and re-mount it which reset the component state. You should keep the layout stable. Please ref [#27263](https://github.com/yuce-design/yuce-design/issues/27263) for more info.
