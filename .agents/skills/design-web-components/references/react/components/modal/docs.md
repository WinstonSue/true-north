...
| closeIcon | Custom close icon. 5.7.0: close button will be hidden when setting to `null` or `false` | ReactNode | &lt;X /> |  | 5.14.0 |
...
| width | Width of the modal dialog | string \| number \| [Breakpoint](../grid/docs.md#col) | 520 | Breakpoint: 5.23.0 | × |
...
| icon | Custom icon | ReactNode | &lt;CircleAlert /> |  |
| keyboard | Whether support press esc to close | boolean | true |  |
...
> [App Package Component](../app/docs.md) can be used to simplify the problem of `useModal` and other methods that need to manually implant contextHolder.
