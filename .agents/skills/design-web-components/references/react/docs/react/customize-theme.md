...
Yuce Design allows you to customize our design tokens to satisfy UI diversity from business or brand requirements, including primary color, border radius, border color, etc.
...
Starting from 6.0.0, we provide `zeroRuntime` mode to further improve application performance. When enabled, Yuce Design will no longer generate component styles at runtime, so you need to manually import the style files.
...
`@sue/design-web-react/dist/sue.css` contains all antd component styles, but does not include hashed className. If you want to import fewer styles, or cannot use the default styles due to configuration changes like `prefix`, we recommend using [@yuce-design/static-style-extract](https://github.com/yuce-design/static-style-extract) to generate static styles.

```tsx
import fs from 'fs';
import { extractStyle } from '@yuce-design/static-style-extract';
...
```jsx
import { theme } from '@sue/design-web-react';
...
```tsx
import type { ThemeConfig } from '@sue/design-web-react';
import { theme } from '@sue/design-web-react';
