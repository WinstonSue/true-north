...
| `colorSuccess` | `string` | Seed | Used to represent the token sequence of operation success, such as Progress and status feedback patterns. |
...
| `colorError` | `string` | Seed | Used to represent the visual elements of the operation failure, such as error Button states and status feedback patterns. |
...
| `fontFamilyCode` | `string` | Seed | Code font, used for code, pre and kbd elements in EditableText |
...
| `fontSizeHeading6` | `string \| number` | Map | Font size of h6 tag. |
...
| `lineHeightHeading6` | `number` | Map | Line height of h6 tag. |
