...
- 插件贡献（IPC、Workflow commands/events/interactions、MCP tools/resources/prompts、Skills、agent instructions、composer mentions，以及 renderer 的 catalog/views/hub/workspaces/actions/shell/locales/scopes/openers/host actions）都写入对应进程的 registry。`StorageRegistry` 与 Agent 运行时定义仍独立。
...
- 插件通过 `host.ai.start` 传入资源 URI。宿主只持久化/转发这些值，打开详情走 `platform.openResource(uri)`。
