...
| 当前任务视图 | 对齐中 | overdue **仅** `isSelectedToday`（见 `task-today/index.tsx`）；列表项为 **execution row**：行内完成+专注，更多菜单保留 start/pause/abandon/delete（`TaskList/TaskItem.tsx`）。 |
| 任务日历与全部视图 | 对齐中 | 月历 chip **左边框**风格；格子悬停新建已落地（`task-calendar/CalendarCell.tsx`）；全部任务筛选能力维持 Desktop。 |
