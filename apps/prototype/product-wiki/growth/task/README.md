...
| 对象 | 产品状态 | 原型覆盖 | 最近变更 |
| --- | --- | --- | --- |
| 任务 | 路线图 | 完整覆盖 | v0.1.0 2026-07-28: 建立产品对象基线。 |
...
|  | `status` | enum | 是 | todo / doing / blocked / done / abandoned | 路线图 | 完整覆盖 | 任务执行状态。 |
...
| 实体：任务 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 完整覆盖 | 建立产品对象基线。 |
| 字段：任务.关联目标 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 完整覆盖 | 建立字段基线。 |
| 字段：任务.父任务 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 完整覆盖 | 建立字段基线。 |
| 字段：任务.状态 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 完整覆盖 | 建立字段基线。 |
| 字段：任务.预估时间 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 完整覆盖 | 建立字段基线。 |
| 视图：本周任务 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 部分覆盖 | 建立视图基线。 |
| 视图：全部任务 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 部分覆盖 | 建立视图基线。 |
| 视图：月度排程 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 部分覆盖 | 建立视图基线。 |
| 视图：任务详情 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 部分覆盖 | 建立 prototype 抽屉覆盖基线。 |
| 规则：关联互斥 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 部分覆盖 | 建立规则基线。 |
| 规则：时间继承 | v0.1.0 | 2026-07-28 | baseline | 路线图 | 部分覆盖 | 建立规则基线。 |
...
任务管理将目标转化为可计划的工作单元。用户借此安排本周工作、查看积压事项，并把完成结果与上层方向连接起来。

<!-- product-ref: growth.task.structure -->
## 业务流程

用户创建任务后选择关联目标或父任务，设置计划时间和优先级，并在执行过程中更新状态。任务可以拆分为子任务；待办和专注时间可围绕任务持续记录，使计划与真实投入保持可追踪。

<!-- product-ref: growth.task.view.week -->
### 本周任务

本周任务把当前周内需要推进的工作集中展示，便于确定每日优先顺序。原型覆盖基本浏览、状态切换与编辑上下文。
